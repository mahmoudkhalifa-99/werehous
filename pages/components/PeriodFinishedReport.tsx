...
import { 
    ...
    ChevronDown, LayoutGrid, Clock, CalendarDays, History, Calculator, Link as LinkIcon, Palette, X
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in font-cairo" dir="rtl">
        ...
        <div className="bg-white rounded-[3.5rem] border-2 border-black shadow-premium overflow-hidden">
            <div className="overflow-x-auto max-h-[65vh] origin-top-right transition-all duration-300" style={{ zoom: pageScale / 100 }}>
                <table className="w-full text-center border-collapse min-w-[3000px]" ref={tableRef}>
                    <thead className="bg-slate-900 text-yellow-400 h-16 sticky top-0 z-40 shadow-lg">
                        <tr className="text-[11px] font-black uppercase">
                            ...
                            <th className="p-3 border border-slate-700 w-16 no-print"><Palette size={16} className="mx-auto"/></th>
                    ...
                    <tbody className="text-[13px] font-bold text-slate-700">
                        {reportData.map((row, idx) => (
                            <tr key={row.product.id} className={`border-b border-black h-12 hover:brightness-95 transition-all`} style={{ backgroundColor: row.product.customFields?.rowColor || (idx % 2 === 0 ? 'white' : '#f8fafc') }}>
                                <td className="p-2 border" style={forceEnNumsStyle}>{idx + 1}</td>
                                <td className="p-2 border no-print"><ColorPicker productId={row.product.id} /></td>
                                <td className="p-2 border text-right pr-8 font-black text-slate-900 text-md">{row.product.name}</td>
                                <td className="p-2 border bg-yellow-50/10" style={forceEnNumsStyle}>{formatEng(row.G)}</td>
                                <td className="p-2 border bg-yellow-50/20" style={forceEnNumsStyle}>{formatEng(row.H)}</td>
